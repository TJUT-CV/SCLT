...
def _save_tracker_output_oxuva(seq: Sequence, tracker: Tracker, output: dict):
    tracker.results_dir2 = "{}/{}".format(tracker.results_dir, seq.dataset)
    if not os.path.exists(tracker.results_dir2):
        os.makedirs(tracker.results_dir2)
    ...
    pred_file = os.path.join(tracker.results_dir2, '{}_{}.csv'.format(vid_id, obj_id))
...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    # tracker.results_dir2="{}/{}".format(tracker.results_dir,seq.dataset)
    save_results="{}/{}/eval_results".format(tracker.results_dir,seq.dataset)
    save_time="{}/{}/time".format(tracker.results_dir,seq.dataset)
    if not os.path.exists(save_results):
        os.makedirs(save_results)

    if not os.path.exists(save_time):
        os.makedirs(save_time)

    base_results_path_results = os.path.join(save_results, seq.name)
    base_results_path_times = os.path.join(save_time, seq.name)
        ...
        if key == 'target_bbox':
            if isinstance(data[0], (dict, OrderedDict)):
                ...
                for obj_id, d in data_dict.items():
                    bbox_file = '{}_{}.txt'.format(base_results_path_results, obj_id)
                    save_bb(bbox_file, d)
            else:
                # Single-object mode
                bbox_file = '{}.txt'.format(base_results_path_results)
            ...
            if isinstance(data[0], dict):
                ...
                for obj_id, d in data_dict.items():
                    timings_file = '{}_{}_time.txt'.format(base_results_path_times, obj_id)
                    save_time(timings_file, d)
            else:
                timings_file = '{}_time.txt'.format(base_results_path_times)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None, num_gpu=2):
    """Runs a tracker on a sequence."""
    # try:
    #     worker_name = multiprocessing.current_process().name
    #     worker_id = int(worker_name[worker_name.find('-'):])
...
def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None, num_gpus=2):
    ...
    if mode == 'sequential':
        for seq in dataset:
            count_seq+=1
            print('{}/{} {}'.format(count_seq,len(dataset),seq.name))
