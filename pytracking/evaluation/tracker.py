...
class Tracker:
    ...
    def temp_Iou(self,rec1, rec2):
        x1, y1, x2, y2 = rec1  # 分别是第一个矩形左右上下的坐标
        x3, y3, x4, y4 = rec2  # 分别是第二个矩形左右上下的坐标
        w1 = x2 - x1  # 第一个矩形的宽
        w2 = x4 - x3  # 第二个矩形的宽
        h1 = y2 - y1
        h2 = y4 - y3
        ...
        min_w = min(x1, x2, x3, x4)
        max_w = max(x1, x2, x3, x4)
        min_h = min(y1, y2, y3, y4)
        max_h = max(y1, y2, y3, y4)

        W = np.maximum(min_w + w1 + w2 - max_w, 0)  # 交叉部分的宽
        H = np.maximum(min_h + h1 + h2 - max_h, 0)  # 交叉部分的高
        Area = W * H  # 交叉的面积
    ...
    def run_sequence(self, seq, visualization=None, debug=None, visdom_info=None, multiobj_mode=None):
        ...
        return output
    def save_confidence(self,seq_name):
        max_score_map_result_path = '{}/{}/longterm/{}'.format(self.results_dir, 'SCLT',seq_name)
        mkdir(max_score_map_result_path)
        max_score_map_confidence_file = open('{}/{}_001_confidence.value'.format(max_score_map_result_path, seq_name),'w')
        max_score_map_confidence_file.write('\n')
        run_time_file = open('{}/{}_001_time.value'.format(max_score_map_result_path, seq_name),'w')
        run_time_file.write('{}\n'.format(self.all_cost_time[0]))
        max_score_map_pre_text = open('{}/{}_001.txt'.format(max_score_map_result_path,seq_name), 'w')
        max_score_map_pre_text.write('1\n')
        for i in range(len(self.all_max_map_array)):
            max_score_map_confidence_file.write('{}\n'.format(self.all_max_map_array[i]))
            max_score_map_pre_text.write(
                '{:.2f},{:.2f},{:.2f},{:.2f}\n'.format(self.all_pre_box[i][0], self.all_pre_box[i][1],
                                                       self.all_pre_box[i][2], self.all_pre_box[i][3]))
            run_time_file.write('{}\n'.format(self.all_cost_time[i+1]))
        max_score_map_confidence_file.close()
        max_score_map_pre_text.close()
        run_time_file.close()
    def _track_sequence(self, tracker, seq, init_info):
        ...
        self.all_cost_time=[]
        self.all_cost_time.append(time.time() - start_time)
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            self.all_pre_box.append(out['target_bbox'])
            self.all_max_map_array.append(out['confidence'])
            prev_output = OrderedDict(out)
        ...
        return output
    def run_vot2021_LT(self, debug=None, visdom_info=None, dataset_path=None):
        params = self.get_parameters()
        ...
        while True:
            ...
            # out = tracker.track(image, info)
            # out,scores = tracker.track(image, info)  # superdimp
            out = tracker.track(image, info) # slot
            prev_output = OrderedDict(out)
